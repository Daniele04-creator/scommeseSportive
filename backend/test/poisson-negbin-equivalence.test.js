 ...
 * Binomial PMFs.
 *
 * There are currently TWO independent implementations in the codebase:
 *   - src/models/utils/MathUtils.ts  (Lanczos logGamma, used by the models)
 *   - src/api/routes.ts              (iterative product / log-sum, module-local,
 *                                     used to format the prediction API payload)
 *
 * The routes.ts functions are not exported, so the exact current algorithm is
 * reproduced verbatim below. These tests answer one question: are the two
 * implementations interchangeable?
 *
 * Conclusion (see assertions):
 *   - On the REALISTIC input domain (integer k >= 0, mu/lambda > 0, 0 < r <= 500)
 *     they are equivalent to ~1e-13, far below the 4-decimal precision the API
 *     actually emits (toFixed(4) => rounding step 5e-5). Unification would be
 *     numerically safe THERE.
 *   - For NEGATIVE k both now return 0: routes.ts gained a defensive
 *     `if (k < 0) return 0;` guard (previously it returned a spurious P(0)),
 *     matching the guard MathUtils already had.
 *   - For invalid/huge r they still DIVERGE BY DESIGN (point mass vs Poisson
 *     fallback). Those divergences are pinned but intentionally left as-is.
 *
 * The two implementations are NOT unified; only the negative-k guard was added.
 */

// --- Verbatim copy of the current src/api/routes.ts implementations ---------
function routesPoissonPMF(k, lambda) {
  if (k < 0) return 0; // defensive guard, mirrors routes.ts
...
function routesNegBinPMF(k, mu, r) {
  if (k < 0) return 0; // defensive guard, mirrors routes.ts
...
test('poissonPMF: MathUtils matches routes.ts for integer k>=0', () => {
  ...
  for (const lambda of LAMBDAS) {
    for (let k = 0; k <= 30; k++) {
      const diff = Math.abs(routesPoissonPMF(k, lambda) - MathUtils.poissonPMF(k, lambda));
...
test('negBinPMF: MathUtils matches routes.ts for integer k>=0 and 0<r<=500', () => {
  ...
  for (const mu of MUS) {
    for (const r of REALISTIC_R) {
      for (let k = 0; k <= 40; k++) {
        const diff = Math.abs(routesNegBinPMF(k, mu, r) - MathUtils.negBinPMF(k, mu, r));
        maxDiff = Math.max(maxDiff, diff);
      }
    }
  }
...
test('negative k returns 0 in both routes.ts and MathUtils', () => {
  for (const k of [-1, -2, -10, -0.5]) {
    assert.equal(routesPoissonPMF(k, 2.5), 0, `routes poisson(${k}) must be 0`);
    assert.equal(MathUtils.poissonPMF(k, 2.5), 0, `MathUtils poisson(${k}) must be 0`);
    assert.equal(routesNegBinPMF(k, 2.5, 12), 0, `routes negbin(${k}) must be 0`);
...
test('negative k guard does not affect k>=0 (regression against the fix)', () => {
  ...
  for (const lambda of [0.5, 2.5, 8]) {
    for (const k of [0, 1, 3, 7]) {
      assert.ok(routesPoissonPMF(k, lambda) > 0, `poisson(${k},${lambda}) stays positive`);
    }
  }
  for (const k of [0, 1, 3, 7]) {
    assert.ok(routesNegBinPMF(k, 2.5, 12) > 0, `negbin(${k}) stays positive`);
  }
});
...
test('DIVERGENCE (by design): invalid r (NaN/Infinity/<=0) — routes.ts degenerates, MathUtils falls back to Poisson', () => {
  ...
  for (const badR of [NaN, Infinity, -5, 0]) {
    ...
    assert.equal(routesNegBinPMF(k, mu, badR), 0, `routes negbin degenerate for r=${badR}`);
...
test('DIVERGENCE (by design): r>500 — MathUtils switches to Poisson, routes.ts keeps exact NegBin (> API precision)', () => {
  const mu = 2.5;
  const k = 2;
  const routes = routesNegBinPMF(k, mu, 600);        // exact NegBin
