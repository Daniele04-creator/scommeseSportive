import { poissonPMF } from '../utils/MathUtils';
...
export class PlayerCardsModel {
  ...
  predictPlayerYellowCards(input: PlayerYellowCardsInput): PlayerYellowCardsPrediction {
    ...
    const rawRate = playerMinutes > 0
      ...
      : rolePrior;
    const weight = this.clamp(playerMinutes / stableMinutes, 0, 1);
    ...
    if (Number.isFinite(input.playerFoulsCommittedPer90) && input.playerFoulsCommittedPer90! > 0) {
      const foulMultiplier = this.clamp(1 + (input.playerFoulsCommittedPer90! - 1.3) * 0.08, 0.85, 1.25);
    ...
    const confidence = this.clamp(0.20 + weight * 0.55 + Math.min(0.20, refCoverage / 100), 0.20, 0.95);
    return {
