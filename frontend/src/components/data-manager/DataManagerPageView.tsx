...
import {
  bulkImportMatches, getMatches,
  getUnderstatTeamSeasonStats, getPlayersByTeam, getStatsOverview, getTeams,
} from '../../utils/api';
...
const DataManagerPageView: React.FC = () => {
  ...
  const playersByTeamRef = useRef<Record<string, any[]>>({});
  const seasonStatsByKeyRef = useRef<Record<string, any>>({});
  const seasonStatsLoadingKeyRef = useRef('');
  const selectedTeam = useMemo(
    () => teams.find((t: any) => String(t.team_id) === String(selectedTeamId)) ?? null,
    [selectedTeamId, teams]
  );

  useEffect(() => {
    playersByTeamRef.current = playersByTeam;
  }, [playersByTeam]);

  useEffect(() => {
    seasonStatsByKeyRef.current = seasonStatsByKey;
  }, [seasonStatsByKey]);

  useEffect(() => {
    seasonStatsLoadingKeyRef.current = seasonStatsLoadingKey;
  }, [seasonStatsLoadingKey]);

  const loadMatchesForCompetition = useCallback(async (competition?: string) => {
  ...
  }, [matches.length, matchesCompetition]);
  ...
  useEffect(() => {
    ...
    void loadMatchesForCompetition(targetCompetition);
  }, [competitionFilter, loadMatchesForCompetition, selectedTeam?.competition, teams]);

  const loadPlayers = useCallback(async (teamId: string) => {
    if (!teamId || playersByTeamRef.current[teamId]) return;
    setPlayersLoading(teamId);
    try {
      const res = await getPlayersByTeam(teamId);
      setPlayersByTeam((prev) => {
        if (prev[teamId]) return prev;
        const next = { ...prev, [teamId]: res.data ?? [] };
        playersByTeamRef.current = next;
        return next;
      });
    } catch {
      setPlayersByTeam((prev) => {
  ...
  const handleBulkImport = async () => {
    try {
      ...
      if (!Array.isArray(arr)) {
        toastState.showToast({ tone: 'warning', message: 'Formato non valido' });
        return;
      }
      ...
      toastState.showToast({ tone: 'error', message: getErrorMessage(e, 'Errore import JSON') });
    }
  };

  const competitions = useMemo(
    () => Array.from(new Set(teams.map((t: any) => t.competition).filter(Boolean))),
    [teams]
  );
  const seasons = useMemo(
    () => Array.from(new Set(matches.map((m: any) => String(m.season ?? '').trim()).filter(Boolean)))
  ...
  const filteredMatches = useMemo(
    ...
    [competitionFilter, matches, seasonFilter, yearFilter]
  );

  const filteredTeams = useMemo(
    () => (competitionFilter ? teams.filter((t: any) => t.competition === competitionFilter) : teams),
    [competitionFilter, teams]
  );
  ...
  useEffect(() => {
    ...
    if (Object.prototype.hasOwnProperty.call(seasonStatsByKeyRef.current, key)) return;
    if (seasonStatsLoadingKeyRef.current === key) return;

    seasonStatsLoadingKeyRef.current = key;
    ...
    getUnderstatTeamSeasonStats({ competition, season: currentTeamSeason, teamId })
      ...
      .finally(() => {
        ...
        if (seasonStatsLoadingKeyRef.current === key) {
          seasonStatsLoadingKeyRef.current = '';
        }
      });
  }, [currentTeamSeason, selectedTeam]);
          ...
          <>
            ...
            <div className="dm-overview-grid">
              <div className="dm-overview-card">
                ...
                {statsOverviewError ? (
                  <ErrorBanner message={statsOverviewError} />
                ) : (
                  <>
              ...
              <div className="dm-detail">
                ...
                {matchesLoadError && (
                  <div style={{ marginBottom: 12 }}>
                    <ErrorBanner message={matchesLoadError} />
                  </div>
                )}
...
export default DataManagerPageView;
